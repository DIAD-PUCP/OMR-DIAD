...
def preprocess_image_barcodes(config: Form, src_img: MatLike) -> MatLike:
    ...
    blur_img = cv2.GaussianBlur(src_img, ksize=(5, 5), sigmaX=0)
    ...
    skew = (bottom_left.orientation + top_right.orientation) / 2
    ...
    blur_img = cv2.GaussianBlur(img, ksize=(5, 5), sigmaX=0)
    barcodes = zxingcpp.read_barcodes(blur_img)
    for code in barcodes:
        if code.text == segment.bottom_left.text:
            bottom_left = code
